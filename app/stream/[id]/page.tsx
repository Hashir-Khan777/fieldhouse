...
export default function StreamPage() {
  ...
  const startStreaming = async () => {
    ...
    socket?.emit(
      "broadcaster",
      JSON.parse(localStorage.getItem("broadcasting") || "null")
    );
    console.log(JSON.parse(localStorage.getItem("broadcasting") || "null"));
  ...
  useEffect(() => {
    if (connected && socket) {
      ...
      socket?.on("broadcasting-details", (details: any) => {
        setStream(details);
      });
    }
  }, [connected]);
          ...
          <div className="video-player-container rounded-lg border border-fhsb-green/20 overflow-hidden">
            <div className="w-full h-full flex items-center justify-center bg-black">
              {!localVideo || !remoteVideo ? (
                ...
                <video
                  ref={user?._id === id ? localVideo : remoteVideo}
                  autoPlay
                  controls
                  playsInline
                  width={1280}
